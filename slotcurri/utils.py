...
def make_log_dir(
    ...
    log_path = pathlib.Path(log_dir) / dir_name

    # count = 2
    # while log_path.is_dir():
    log_path = log_path.with_name(f"{dir_name}")
    # count += 1
...
def find_last_checkpoint(log_path: pathlib.Path, experiment_group: str = None) -> pathlib.Path:
    if experiment_group is not None:
        pattern = f"**/*{experiment_group}_step*.ckpt"
    else:
        pattern = "**/*.ckpt"

    checkpoints = list(log_path.glob(pattern))
    if not checkpoints:
        return None
    return max(checkpoints, key=lambda ckpt: ckpt.stat().st_ctime)
#
